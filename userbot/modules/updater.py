...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await event.edit(
                f'{txt}\n`Kredensial Heroku tidak valid untuk deploy SKY-UBOT dyno.`'
            )
            return repo.__del__()
        await event.edit('`[HEROKU]:'
                         '\nSedang MengUpdate SKY-𝐔𝐁𝐎𝐓, Mohon Menunggu 5-7 Menit`'
                         )
        ups_rem.fetch(ac_br)
        ...
        if build.status == "failed":
            ...
            await event.edit("`SKY-UBOT Berhasil Di Deploy!\n" "Restarting, Mohon Tunggu Sebentar.....`")
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('**SKY-𝐔𝐁𝐎𝐓** `Berhasil Di Update!`')
    await asyncio.sleep(1)
    await event.edit('**SKY-𝐔𝐁𝐎𝐓** `Di Restart....`')
    await asyncio.sleep(1)
...
async def upstream(event):
    ...
    if conf is None and force_update is False:
        changelog_str = f'**Pembaruan Untuk SKY-𝐔𝐁𝐎𝐓 :\n\n➣ Pembaruan Data :**\n`{changelog}`'
...
CMD_HELP.update({
    'update':
    "𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.update`"
    "\n• : Untuk Melihat Pembaruan Terbaru SKY-UBOT."
    "\n\nCommand: `.update now`"
